...
#
# Extracts locations from foreign aid project dccuments and finds their matches in GeoNames and sentences throughout the document corpus mentioning these location matches.
...
def removeDuplicates_adv(values):
...
def removeDuplicates_simp(values):
...
def mineLocs(content):
    ...
    return removeDuplicates_simp(nerlocs)
...
def getLocData(name):
    ...
    if len(matches) == 0:
        ...
        if (len(matches) == 0) and (len(fuzzyMatches) == 0):
            ...
            return removeDuplicates_adv(matches + fuzzyMatches)
    else:
        return removeDuplicates_adv(matches)

def getGeonamesMatches(loclist):
    """ return list of location matches found in geonames """
    geonameLocs = []
    for loc in loclist:
        matches = getLocData(loc)
        if matches is not None:
            geonameLocs.extend(matches)
    return removeDuplicates_adv(geonameLocs)
...
def eliminateOutliers(loclist):
    ...
    for x in removeDuplicates_adv(outliers):
        loclist.remove(x)
    return loclist
...
def run(doclist):
    ...
    sents = removeDuplicates_simp(sentsContaining(content, locs3_names))
